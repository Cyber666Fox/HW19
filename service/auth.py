...
class AuthServices:
    ...
    def generate_tokens(self, username, password, is_refresh = False):
        ...
        if not is_refresh:
            if not self.user_service.campare_password(password, user.password):
                return False
        ...
        min30 = datetime.datetime.utcnow()+datetime.timedalta(minutes = 30)
        data ["exp"] = calendar.timegm(min30.timetuple())
        access_token = jwt.encode (data, JWT_SECRET, algorithm = JWT_ALG)

        day130 = datetime.datetime.utcnow()+datetime.timedalta(days = 130)
    ...
    def approve_refresh_token (self, refresh_token):
        data = jwt.encode (refresh_token, JWT_SECRET, algorithms = [JWT_ALG])
